  ...
  for (const elm of elms) {
    for (const node of elm.children) {
      for (const c of node.children) {
        if (!!c.attributes?.href.value) {
          if (subscribersMap.has(c.attributes?.href.value)) {
            c.removeEventListener('click', subscribersMap.get(c.attributes?.href.value))
            subscribersMap.delete(c.attributes?.href.value)
          }
        }
