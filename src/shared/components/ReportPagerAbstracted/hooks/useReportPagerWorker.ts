...
export const useReportPagerWorker = <TTargetResult, TWorkerServiceReport>({
        ...
        switch (true) {
          default:
            switch (e.data.__eType) {
              case NWService.EWorkerToClientEvent.SORTED_REPORT_PAGER_OK:
                ...
                break
              case NWService.EWorkerToClientEvent.SORTED_REPORT_PAGER_ERR:
                if (
  ...
  useLayoutEffect(() => {
    if (isEnabled)
      sendSignalToWorkerFnRef.current({
        input: {
          opsEventType: NWService.EClientToWorkerEvent.GET_SORTED_REPORT_PAGER,
