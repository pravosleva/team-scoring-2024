...
export const useJobsPagerWorker = <TTargetResult, TWorkerServiceReport>({ isEnabled, isDebugEnabled, deps, cb }: TProps<TTargetResult>) => {
  ...
  useLayoutEffect(() => {
        ...
        switch (true) {
          default:
            switch (e.data.__eType) {
              case NWService.EWorkerToClientEvent.JOBS_PAGER_OK:
                if (isDebugEnabled) groupLog({

                  // -- ATTENTION! 2/6

                  namespace: `[useJobsPagerWorker] by jobs-pager (on data #1) [${e.data.__eType}]`,
                ...
                break
              case NWService.EWorkerToClientEvent.JOBS_PAGER_ERR:
                if (
              ...
              default: {
                if (isDebugEnabled) groupLog({
                  ...
                  namespace: `[useJobsPagerWorker] by jobs-pager ⚠️ (on data) UNHANDLED! [${e.data.__eType}`,
    ...
    wws.subscribeOnErr<{
      ...
      cb: (e: MessageEvent<{
        ...
        if (isDebugEnabled) groupLog({
          namespace: '[useJobsPagerWorker] by jobs-pager 🚫 OnErr e:',
          items: [e],
        })
      },
    ...
    // return () => {
    //   const wList = ['jobs-pager']
    //   for (const wName of wList) {
    //     wws.terminate({
    //       wName,
    //       cb: () => {
    //         if (isDebugEnabled) groupLog({ namespace: `[useJobsPagerWorker] by jobs-pager 🚫 die [${wName}]`, items: [] })
    //       },
    //     })
    //   }
    // }
  ...
  useLayoutEffect(() => {
    if (isEnabled)
      sendSignalToWorkerFnRef.current({
        input: {
          opsEventType: NWService.EClientToWorkerEvent.GET_JOBS_PAGER,
      ...
      groupLog({
        namespace: '[useJobsPagerWorker] by jobs-pager 🚫 DISABLED',
        items: [
