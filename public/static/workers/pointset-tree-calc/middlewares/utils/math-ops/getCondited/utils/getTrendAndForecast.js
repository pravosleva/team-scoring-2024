  ...
  if (typeof sensedSpeed === 'number' && sensedSpeed > 0) {
    ...
    result.linearForecast = {
      ...
      roleDescriptions: {
        ...
        manager: `Идеализированный долгосрочный план-график, сглаживающий текущую волатильность и локальные задержки команды. Простыми словами: это прогноз по «крейсерской скорости» проекта за всё время. Он игнорирует тот факт, что на этой неделе кто-то заболел или ушел в отпуск, показывая стабильный средний темп системы.`,
    ...
    result.adaptiveForecast = {
      ...
      roleDescriptions: {
        ...
        manager: `Реалистичная оценка даты завершения работ, основанная на актуальном фокусе и текущей доступности ресурсов команды. Простыми словами: это прогноз по «текущей скорости» прямо сейчас. Если команда последние дни буксует или, наоборот, сильно ускорилась, этот график мгновенно пересчитает дату под новые реалии.`,
