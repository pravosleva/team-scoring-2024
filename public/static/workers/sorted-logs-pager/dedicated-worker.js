...
// console.log('[LOAD:1] sorted-logs-pager/dedicated-worker.js')
...
importScripts('../utils/array-ops/taro-special-tools/getFilteredLogs.js')
          ...
          validate: (val) => {
            ...
            switch (true) {
              ...
              case (
                ...
                || !Object.values(NES.Common.ClientService.SortedLogsPager.EClientToWorkerEvent).includes(val)
              ):
                result.ok = false
      ...
      default: {
        ...
        if (!!input?.opsEventType) {
          ...
          withRootMW({
            eventData: e.data,
            cb: {
              [NES.Common.ClientService.SortedLogsPager.EClientToWorkerEvent.PING_GET]: ({
                output,
                input,
                _service,
              }) => {
                if (debugConfig.workerEvs.fromClient.isEnabled) log({
                  label: [
                    ...
                    `cb[${NES.Common.ClientService.SortedLogsPager.EClientToWorkerEvent.PING_GET}]`,
                  ].join(' -> '),
                ...
                const sendError = () => {
                  self.postMessage({
                    __eType: NES.Common.ClientService.SortedLogsPager.EWorkerToClientEvent.PONG_ERR,
                    data: {
                      _service: {
                ...
                const sendData = () => {
                  self.postMessage({
                    __eType: NES.Common.ClientService.SortedLogsPager.EWorkerToClientEvent.PONG_OK,
                    data: {
                      _service: {
                        tsList: _perfInfo.tsList,
                        ..._service,
