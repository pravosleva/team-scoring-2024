...
importScripts('../utils/delay.js')
importScripts('../utils/compose.js')
importScripts('../utils/clsx.js')
importScripts('../utils/string-ops/getRandomString.js')
importScripts('../utils/string-ops/getMatchedByRegExp.js')
importScripts('../utils/string-ops/getMatchedByAllStrings.js')
importScripts('../utils/string-ops/getMatchedByAnyString.js')
importScripts('../utils/object-ops/mutateObject.js')
...
importScripts('../utils/time-ops/getTimeAgo.js')
importScripts('../utils/array-ops/sort-array-objects@3.0.0/getSortedArray.js')
          ...
          validate: (val) => {
            ...
            switch (true) {
              ...
              case (
                ...
                || !Object.values(NES.Common.ClientService.SarchPagerBasic.EClientToWorkerEvent).includes(val)
              ):
                result.ok = false
      ...
      default: {
        ...
        if (!!input?.opsEventType) {
          ...
          withRootMW({
            eventData: e.data,
            cb: {
              [NES.Common.ClientService.SarchPagerBasic.EClientToWorkerEvent.PING_GET]: ({
                output,
                input,
                _service,
              }) => {
                if (debugConfig.workerEvs.fromClient.isEnabled) log({
                  label: [
                    ...
                    `cb[${NES.Common.ClientService.SarchPagerBasic.EClientToWorkerEvent.PING_GET}]`,
                  ].join(' -> '),
                ...
                const sendError = () => {
                  self.postMessage({
                    __eType: NES.Common.ClientService.SarchPagerBasic.EWorkerToClientEvent.PONG_ERR,
                    data: {
                      _service: {
                ...
                const sendData = () => {
                  self.postMessage({
                    __eType: NES.Common.ClientService.SarchPagerBasic.EWorkerToClientEvent.PONG_OK,
                    data: {
                      _service: {
                        tsList: _perfInfo.tsList,
                        ..._service,
